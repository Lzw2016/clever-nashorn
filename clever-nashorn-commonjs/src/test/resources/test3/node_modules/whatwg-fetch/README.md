...
fetch('/users.json')
  .then(function(response) {
    ...
    logConsole.log('parsed json', json)
  }).catch(function(ex) {
    logConsole.log('parsing failed', ex)
  })
```
...
fetch('/users.json').then(function(response) {
  logConsole.log(response.headers.get('Content-Type'))
  logConsole.log(response.headers.get('Date'))
  logConsole.log(response.status)
  logConsole.log(response.statusText)
})
```
...
fetch('/users')
  ...
  .then(parseJSON)
  .then(function(data) {
    logConsole.log('request succeeded with JSON response', data)
  }).catch(function(error) {
    logConsole.log('request failed', error)
  })
```
